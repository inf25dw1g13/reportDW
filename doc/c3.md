...
Executa scripts específicos de CBD, como 03_audit.sql e 04_setup_replication.sql.  
...
Cada réplica conecta-se ao Master e inicia o processo de replicação até ficarem totalmente sincronizadas.  

**4. Scripts**  
Os scripts são executados apenas na primeira inicialização dos volumes.  
A execução depende do estado saudável do Master.  

**5. Inicialização do MaxScale**  
Lê as configurações definidas no ficheiro .cnf.  
...
SELECT → encaminhado para as réplicas (3307 ou 3308).  
INSERT / UPDATE / DELETE → enviados para o Master (3309).  
...
**RUN chmod +x /docker-entrypoint-initdb.d/*.sh 2>/dev/null || true**        Permite que os bash possam ser executados quando o MariaDB estiver pronto.  
